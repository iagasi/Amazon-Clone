...
useEffect(()=>{
  if(!password){return}
    if(password==password2){setActive(false);console.log("matching");setPassError()}
    else{
        setActive(true)
...
<input   minlength="6" placeholder='min length 6 sumbols' value={password} onChange={(e)=>setPassword(e.target.value)}></input>
<small>Repeat Password</small>
<input  minlength="6" placeholder='min length 6 sumbols' value={password2} onChange={(e)=>setPassword2(e.target.value)}></input>
...
<button  className="register__register"disabled={active}  onClick={register}>Register</button>
<button  className="register__close" onClick={()=>close()}>Close</button>
</div>
